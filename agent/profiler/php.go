...
func (p *PhpProfiler) Invoke(job *details.ProfilingJob) error {
	...
	cmd := exec.Command(phpSpyLocation, "--buffer-size=40000", "--limit=50000", "-p", pid, "-o", phpOutputFileName, "-i", (duration * 1000) )
	...
	err = cmd.Run()
	if err != nil {
		fmt.Println(phpSpyLocation, "--buffer-size=40000", "--limit=50000", "-p", pid, "-o", phpOutputFileName, "-i", (duration * 1000) )
		fmt.Println(err)
		return err
	}
